...
from core.utils import parse_weekday
...
def morning_plan():
    ...
    for t in all_active:
        if t["name"] not in done_yesterday and _task_due_on(t, parse_weekday(yesterday_str)):
    ...
    # 接入 AI 规划建议
    try:
        plan_suggestion = plan.suggest_daily_plan()
        if plan_suggestion:
            msg += f"\n---\n🎯 今日规划建议：\n{plan_suggestion}"
    except Exception as e:
        logger.warning("获取规划建议失败: %s", e)
...
def fitness_reminder():
    notify("🏋️ 健身提醒", "健身时间到！放下手里的活，动一下！")
