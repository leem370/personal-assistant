...
def execute_actions(actions, caller, user_text=None):
    ...
    for act in actions:
        ...
        if intent == "log":
            ...
            # 仅小知语义：自我进化，把新规律增量沉淀进画像
            replies.append(_do_learn(act, caller))
        elif intent == "chat":
            replies.append(act.get("reply", "嗯嗯，我在。"))
        else:
            replies.append(act.get("reply", "收到。"))

    # 后备：AI 偶尔不按 learn 意图走，而是把 <<<UPDATE>>> 标记塞进 reply 文本。
    # 这里扫一遍，抽出来落库并把标记从给用户看的正文里剥掉。
    replies = _harvest_update_markers(replies)
...
def _do_log(act):
    ...
    comp = max(0, min(100, int(act.get("completion_rate", 100) or 100)))
    focus = max(0, min(10, int(act.get("focus_score", 0) or 0)))
    note = act.get("note", "")
    mood = act.get("mood")
...
def _do_learn(act, caller):
    """自我进化：把 AI 发现的新成功策略 / 失败陷阱增量沉淀进画像。

    new_strategy / new_trigger 均可选，至少一个有效才会落库。返回极简一句
    不干扰主回复——通常和小知正文回复拼在一起，放在末尾。
    """
    new_strategy = (act.get("new_strategy") or "").strip()
    new_trigger = (act.get("new_trigger") or "").strip()
    saved = False
    if new_strategy:
        db.append_profile_strategy("success_strategies", new_strategy)
        saved = True
    if new_trigger:
        db.append_profile_strategy("failure_triggers", new_trigger)
        saved = True
    # 有显式 reply 就用，否则给一句极简的内部沉淀提示
    if saved:
        return act.get("reply") or "（已记下这条规律，下次会主动提醒你）"
    return act.get("reply", "")


def _harvest_update_markers(replies):
    """后备路径：扫所有 reply 文本，抽取 <<<UPDATE>>> 指令落库并剥掉标记。

    首选路径是 learn 意图；这里是兜底，防 AI 把标记塞进 reply。
    解析失败静默跳过，不影响主流程。返回清洗后的 replies 列表。
    """
    if not any("<<<UPDATE>>>" in r for r in replies):
        return replies
    cleaned = []
    for r in replies:
        if "<<<UPDATE>>>" not in r:
            cleaned.append(r)
            continue
        update = profile.parse_update_marker(r)
        if update:
            if update.get("new_strategy"):
                db.append_profile_strategy("success_strategies", update["new_strategy"])
            if update.get("new_trigger"):
                db.append_profile_strategy("failure_triggers", update["new_trigger"])
        stripped = profile.strip_update_marker(r)
        if stripped:
